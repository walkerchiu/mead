 ...
 *  - 0–27%（0–1000ms）：被點擊的卡 + 主標整句（黑字 + 橘字 + 小標）一起
 *    往上抽離（1.0s 的「整句往上飄」）。
 *  - 27–40%（1000–1500ms）：停 0.5s（橘字「抓住一下」的停頓）。
 *  - 40–67%（1500–2500ms）：黑字 + 小標 + 被點擊的卡繼續上升 1s、淡出視野。
 *                                     橘字保持在第一段落點不動。
 *  - 67–81%（2500–3000ms）：橘字單獨存在 0.5s（強調其「主角」身分，
 *                                     被點擊的卡仍停在 -120 等候）。
 *  - 81–89%（3000–3300ms）：被點擊的卡顏色變淺、放大到 3x、淡出 0.3s；
 *                                     同時段橘字也淡出。
    ...
    <Box sx={{ textAlign: 'center', px: 3 }}>
      ...
      <Typography
        ...
        sx={{
          ...
          // Slogan exit on click：
          //  - 整句先一起向上抽離 0 → -120px（0–38%）。
          //  - 在第二屏邊緣停 0.5s（38–68%）。
          //  - 黑字（「讓」/「被看見」）繼續上升至 -380px 並淡出（68–88%）。
          //  - 橘字（keyword）守在 -120px 不動，最後一段（88–100%）才淡出。
