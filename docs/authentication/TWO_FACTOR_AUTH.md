...
- [雙因素認證 (2FA) 文檔](#雙因素認證-2fa-文檔)
  ...
  - [❓ 常見問題](#-常見問題)
    ...
    - [Q4: 可以強制某些使用者必須啟用 2FA 嗎？](#q4-可以強制某些使用者必須啟用-2fa-嗎)
...
雙因素認證（Two-Factor Authentication, 2FA）是一種額外的安全層，在使用者輸入密碼後，要求提供第二個驗證因素來確認身份。本系統目前支援 **Email-based 2FA**，並預留了擴展性以支援 TOTP 和 SMS。
...
### Q4: 可以強制某些使用者必須啟用 2FA 嗎？

**A**: 目前不支援。但可在未來版本中實現強制 ADMIN_SCOPE 使用者啟用 2FA。
...
**A**: 目前不支援。每個使用者只能啟用一種 2FA 方式（EMAIL, TOTP, 或 SMS）。
