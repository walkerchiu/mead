...
{$SITE_DOMAIN} {
 ...
 reverse_proxy frontend:3000 {
  ...
  # 正規化回應 Location 標頭，避免對外 URL 帶 :3000。
...
> 3000 不對外開 → 使用者會連不上。Caddy 會將 Location 的 `:3000` 正規化為對外網域路徑。
...
3. **HSTS preload**：`next.config.ts` 輸出 `Strict-Transport-Security: max-age=63072000; includeSubDomains`（**不含 `preload`**）。
