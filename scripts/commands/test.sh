...
if [ "$I18N_ONLY" = true ]; then
  ...
  log_info "▶ 後端 i18n 測試"
  pnpm --filter @starter/backend test:i18n

  echo ""
  log_info "▶ 前端 i18n 測試"
  pnpm --filter @starter/frontend test:i18n
  ...
  if pnpm --filter @starter/backend type-check; then
    log_success "型別檢查通過"
  else
    log_error "型別檢查失敗"
    exit 1
  ...
  log_info "▶ 2/2 單元測試"
  if [ "$WATCH" = true ]; then
    pnpm --filter @starter/backend test:watch
  elif [ "$COVERAGE" = true ]; then
    pnpm --filter @starter/backend test:cov
  else
    pnpm --filter @starter/backend test
  fi
  ...
  # TypeScript 型別檢查
  log_info "▶ 1/2 TypeScript 型別檢查"
  if pnpm --filter @starter/frontend type-check; then
    log_success "型別檢查通過"
  else
    log_error "型別檢查失敗"
    exit 1
  ...
  log_info "▶ 2/2 單元測試"
  if [ "$WATCH" = true ]; then
    pnpm --filter @starter/frontend test -- --watch
  elif [ "$COVERAGE" = true ]; then
    pnpm --filter @starter/frontend test -- --coverage
  else
    pnpm --filter @starter/frontend test
  fi
